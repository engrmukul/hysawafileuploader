...
class SpSiV2PhysicalSearch
{
    ...
    private function process()
    {
        ...
        $q = SPSanAnswerObs::with('SPSanInspectionV2', 'SPSanInspectionV2.infrastructure.school', 'SPSanInspectionV2.infrastructure.school.district',
            ...
            ->where(function($query) use($request) {
                ...
                if(auth()->user()->roles()->first()->name == 'sp_survey_supervisor'){
                    $query->whereHas('SPSanInspectionV2.infrastructure.school', function($query2) use($distid)
                    {
                ...
                {
                    $query->whereHas('SPSanInspectionV2.infrastructure.school', function($query2) use($upid)
                    {
                ...
                {
                    $query->whereHas('SPSanInspectionV2.infrastructure.school', function($query2) use($unid)
                    {
                ...
                {
                    $query->whereHas('SPSanInspectionV2.infrastructure.school', function($query2) use($institution_type)
                    {
                ...
                {
                    $query->whereHas('sp_san_inspection_v2.infrastructure', function($query2) use($tech_type)
                    {
                ...
                {
                    $query->where('sp_san_inspection_v2.quarter', '=', $quarter);
                }

                if(!empty($year) && $year != 'all' && $year != null)
                {
                    $query->where('sp_san_inspection_v2.year', '=', $year);
                }

            })
        //->where('created_by', '=', auth()->user()->id)
        //->groupBy('id');
        ->whereHas('SPSanInspectionV2.infrastructure', function($query2)
        {
            if(auth()->user()->roles()->first()->name == 'sp_survey_supervisor'){
        ...
        ->whereHas('SPSanInspectionV2', function($query2)
        {
            $query2->orderBy('sp_san_inspection_v2.inspection_date', 'desc');
        });
