...
class RenovationV2Download
{
  ...
  public function download()
  {
    ...
    \Excel::create(date("d-m-Y").' SafePani Renovation List', function($excel) use($rows) {
      $excel->sheet('Sheetname', function($sheet) use($rows) {
        ...
        $sheet->row(1, array(
          ...
          'Month',
          'Repair work details',
          'TW platform renovation',
          'Small parts replacement',
          'Large parts replacement',
          'Drainage with soak well',
          'Labour cost',
          'Material transport cost',
          'Catchment area renovation',
          'Pipes',
          'Tank and tap stand',
          'Filter media renovation',
          'Submersible pump',
          'Vessel media and Dosing',
          'Total cost',
          'Comments',
          'Actively Managed'
          )
        ...
        foreach($rows as $row)
        {
            ...
            $row['month'],
            $row['repair_details'],
            $row['tw_platform_renov'],
            $row['replace_small_parts'],
            $row['replace_large_parts'],
            $row['drainage_soak_well'],
            $row['labour_cost'],
            $row['mat_transport_cost'],
            $row['renov_catchment'],
            $row['pipes'],
            $row['tank_tap_stand'],
            $row['renov_filter'],
