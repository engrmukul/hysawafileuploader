...
class MonthlyReportGeneratorAdmin
{
  private $reportData;
  ...
  public function getReport()
  {
    ...
    if($this->report_type == "print")
    {
      ...
      return view($this->view_path.'print',
        compact('ReportData','row_lastdata','row_totaldata','row_getbaseline','row_gettargets', 'period', 'reportHeader'));
    ...
    }else
    {
       ...
       $request = $this->request;
        $districts = District::where('region_id', $request->input('region_id'))->get();
  ...
  private function upazilaAllReportAll(Request $request)
  {
   // dd("up1");
    $request = $this->request;
    //dd("dd1"+$request->input('district_id'));
  ...
  private function upazilaOneReportAll(Request $request)
  {
    ...
    $request = $this->request;
    //dd("dd3"+$request->input('district_id'));
    $id = $request->input('upazila_id');
    $type = "upid";
    $last_report_id = $this->_getLastReportId($type, $id);
    $this->row_getbaseline = $this->_getBaseLineData($id, $type);
  ...
  private function upazilaOneReportOne(Request $request)
  {
    ...
    $request = $this->request;
    //dd("dd4"+$request->input('district_id'));
    $last_report_id = $request->input('rep_data_id');
    $id = $request->input('upazila_id');
    $type = "upid";
    $this->row_getbaseline = $this->_getBaseLineData($id, $type);
  ...
  // dd('My2.1');
    $request = $this->request;
  //dd($request->has('project_id'));


    $query = $this->REPORTDATAQUERYSELECT ." FROM rep_data WHERE rep_id = 99 and proj_id = ".$request->input('project_id')."";
   
    if($id != null){
      $query = $this->REPORTDATAQUERYSELECT ." FROM rep_data WHERE rep_id = 99 and $type=$id and proj_id =".$request->input('project_id')."";
     }
  ...
  //  dd('My3.1');
    ...
    if($id != null){
      $query = $this->REPORTDATAQUERYSELECT ." FROM rep_data WHERE rep_id = 100  and $type=$id and proj_id =".$request->input('project_id')."" ;
    }
  ...
  {
    $request = $this->request;
    $query = $this->REPORTDATAQUERYSELECT . " FROM rep_data WHERE rep_id < $last_report_id and proj_id =".$request->input('project_id')."";

    if($id != null){
      $query = $this->REPORTDATAQUERYSELECT. " FROM rep_data WHERE rep_id < $last_report_id  AND $type=$id and proj_id =".$request->input('project_id')."" ;
    }
  ...
  private function _getTotalData($last_report_id, $id = null, $type = "unid")
  {
    ...
    // if($type !=null){
    //   $query = $this->REPORTDATAQUERYSELECT . " FROM rep_data WHERE rep_id <= $last_report_id and proj_id =".auth()->user()->proj_id." and unid=".$this->request->input('union_id')."" ;
  ...
  private function reportHeader()
  {
    ...
    if($this->request->has('union_id') && $this->request->union_id != "")
    {
      ...
      if($this->request->union_id == "all")
      {
        ...
        if($role == "upazila_admin"){
          $upazila = Upazila::with('district.region')->find($request->input('upazila_id'));
        }else{
