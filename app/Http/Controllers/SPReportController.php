...
class SPReportController extends Controller
{
    ...
    public function weeklyWqAndSiReport(Request $request)
    {
        ...
        $numericKeys = array_filter($numericKeys, function ($k) {
            return $k !== 'name';
        });
    ...
    private function combinedSql($fromDate = null, $toDate = null, $upazilaId = null): string
    {
        ...
        $siDateCondition = ($fromDate && $toDate) ? "AND si2.inspection_date BETWEEN :from_si AND :to_si" : "";
        $siTechDateCondition = ($fromDate && $toDate) ? "AND si2.inspection_date BETWEEN :from_si_tech AND :to_si_tech" : "";

        // Build date condition for sample collection
        $scDateCondition = ($fromDate && $toDate) ?
            "AND sc.sample_date >= :from_sc AND sc.sample_date < DATE_ADD(:to_sc, INTERVAL 1 DAY)" : "";

        $bfDateCondition = ($fromDate && $toDate) ?
            "AND sc.sample_date >= :from_bf AND sc.sample_date < DATE_ADD(:to_bf, INTERVAL 1 DAY)" : "";

        $ecdDateCondition = ($fromDate && $toDate) ?
            "AND sc.sample_date >= :from_ecd AND sc.sample_date < DATE_ADD(:to_ecd, INTERVAL 1 DAY)" : "";

        $disDateCondition = ($fromDate && $toDate) ?
            "AND sc.sample_date >= :from_dis AND sc.sample_date < DATE_ADD(:to_dis, INTERVAL 1 DAY)" : "";

        $bsDateCondition = ($fromDate && $toDate) ?
            "AND sc.sample_date >= :from_bs AND sc.sample_date < DATE_ADD(:to_bs, INTERVAL 1 DAY)" : "";

        $chemDateCondition = ($fromDate && $toDate) ?
            "AND sc.sample_date >= :from_chem AND sc.sample_date < DATE_ADD(:to_chem, INTERVAL 1 DAY)" : "";

        $csDateCondition = ($fromDate && $toDate) ?
            "AND sc.sample_date >= :from_cs AND sc.sample_date < DATE_ADD(:to_cs, INTERVAL 1 DAY)" : "";

        $arsenicDateCondition = ($fromDate && $toDate) ?
            "AND sc.sample_date >= :from_as AND sc.sample_date < DATE_ADD(:to_as, INTERVAL 1 DAY)" : "";

        $manganeseDateCondition = ($fromDate && $toDate) ?
            "AND sc.sample_date >= :from_mn AND sc.sample_date < DATE_ADD(:to_mn, INTERVAL 1 DAY)" : "";

        $chlorideDateCondition = ($fromDate && $toDate) ?
            "AND sc.sample_date >= :from_cl AND sc.sample_date < DATE_ADD(:to_cl, INTERVAL 1 DAY)" : "";
...
arsenic_tech AS (
  SELECT s.upid,
    SUM(CASE WHEN i.tech_type='DTW' AND sc.sample_cat='Chemical' AND wq.parameter='Arsenic' AND wq.value >= 0.05 THEN 1 ELSE 0 END) AS arsenic_dtw,
    SUM(CASE WHEN i.tech_type='STW' AND sc.sample_cat='Chemical' AND wq.parameter='Arsenic' AND wq.value >= 0.05 THEN 1 ELSE 0 END) AS arsenic_stw,
    SUM(CASE WHEN i.tech_type='RWH' AND sc.sample_cat='Chemical' AND wq.parameter='Arsenic' AND wq.value >= 0.05 THEN 1 ELSE 0 END) AS arsenic_rwhf,
    SUM(CASE WHEN i.tech_type='PWS' AND sc.sample_cat='Chemical' AND wq.parameter='Arsenic' AND wq.value >= 0.05 THEN 1 ELSE 0 END) AS arsenic_pws
  FROM sp_sample_collection sc
  JOIN sp_water_quality wq ON wq.sample_id = sc.sample_id
  JOIN sp_infrastructure i ON i.id = sc.infrastructure_id AND i.is_active = 1
  JOIN sp_school s ON s.id = i.school_id
  WHERE s.distid = :d12 {$arsenicDateCondition}
  GROUP BY s.upid
),
manganese_tech AS (
  SELECT s.upid,
    SUM(CASE WHEN i.tech_type='DTW' AND sc.sample_cat='Chemical' AND wq.parameter='Manganese' AND wq.value >= 0.4 THEN 1 ELSE 0 END) AS manganese_dtw,
    SUM(CASE WHEN i.tech_type='STW' AND sc.sample_cat='Chemical' AND wq.parameter='Manganese' AND wq.value >= 0.4 THEN 1 ELSE 0 END) AS manganese_stw,
    SUM(CASE WHEN i.tech_type='RWH' AND sc.sample_cat='Chemical' AND wq.parameter='Manganese' AND wq.value >= 0.4 THEN 1 ELSE 0 END) AS manganese_rwhf,
    SUM(CASE WHEN i.tech_type='PWS' AND sc.sample_cat='Chemical' AND wq.parameter='Manganese' AND wq.value >= 0.4 THEN 1 ELSE 0 END) AS manganese_pws
  FROM sp_sample_collection sc
  JOIN sp_water_quality wq ON wq.sample_id = sc.sample_id
  JOIN sp_infrastructure i ON i.id = sc.infrastructure_id AND i.is_active = 1
  JOIN sp_school s ON s.id = i.school_id
  WHERE s.distid = :d13 {$manganeseDateCondition}
  GROUP BY s.upid
),
chloride_tech AS (
  SELECT s.upid,
    SUM(CASE WHEN i.tech_type='DTW' AND sc.sample_cat='Chemical' AND wq.parameter='Chloride'  AND wq.value >= 1000 THEN 1 ELSE 0 END) AS chloride_dtw,
    SUM(CASE WHEN i.tech_type='STW' AND sc.sample_cat='Chemical' AND wq.parameter='Chloride'  AND wq.value >= 1000 THEN 1 ELSE 0 END) AS chloride_stw,
    SUM(CASE WHEN i.tech_type='RWH' AND sc.sample_cat='Chemical' AND wq.parameter='Chloride'  AND wq.value >= 1000 THEN 1 ELSE 0 END) AS chloride_rwhf,
    SUM(CASE WHEN i.tech_type='PWS' AND sc.sample_cat='Chemical' AND wq.parameter='Chloride'  AND wq.value >= 1000 THEN 1 ELSE 0 END) AS chloride_pws
  FROM sp_sample_collection sc
  JOIN sp_water_quality wq ON wq.sample_id = sc.sample_id
  JOIN sp_infrastructure i ON i.id = sc.infrastructure_id AND i.is_active = 1
  JOIN sp_school s ON s.id = i.school_id
  WHERE s.distid = :d14 {$chlorideDateCondition}
  GROUP BY s.upid
)
SELECT
  b.upname AS name,
  ...
  COALESCE(at.arsenic_dtw,0)  AS arsenic_dtw,
  COALESCE(at.arsenic_stw,0)  AS arsenic_stw,
  COALESCE(at.arsenic_rwhf,0) AS arsenic_rwhf,
  COALESCE(at.arsenic_pws,0)  AS arsenic_pws,
  COALESCE(mt.manganese_dtw,0)  AS manganese_dtw,
  COALESCE(mt.manganese_stw,0)  AS manganese_stw,
  COALESCE(mt.manganese_rwhf,0) AS manganese_rwhf,
  COALESCE(mt.manganese_pws,0)  AS manganese_pws,
  COALESCE(clt.chloride_dtw,0)  AS chloride_dtw,
  COALESCE(clt.chloride_stw,0)  AS chloride_stw,
  COALESCE(clt.chloride_rwhf,0) AS chloride_rwhf,
  COALESCE(clt.chloride_pws,0)  AS chloride_pws
FROM base_upazila b
...
LEFT JOIN arsenic_tech   at  ON at.upid  = b.upid
LEFT JOIN manganese_tech mt  ON mt.upid  = b.upid
LEFT JOIN chloride_tech  clt ON clt.upid = b.upid
ORDER BY b.upname
";
    ...
    SELECT
      ...
      /* Arsenic exceed by tech */
      COALESCE(SUM(CASE WHEN sc.sample_cat='Chemical' AND i.tech_type='DTW' AND wq.parameter='Arsenic'   AND wq.value >= 0.05  THEN 1 ELSE 0 END), 0) AS as_dtw,
      COALESCE(SUM(CASE WHEN sc.sample_cat='Chemical' AND i.tech_type='STW' AND wq.parameter='Arsenic'   AND wq.value >= 0.05  THEN 1 ELSE 0 END), 0) AS as_stw,
      COALESCE(SUM(CASE WHEN sc.sample_cat='Chemical' AND i.tech_type='RWH' AND wq.parameter='Arsenic'   AND wq.value >= 0.05  THEN 1 ELSE 0 END), 0) AS as_rwhf,
      COALESCE(SUM(CASE WHEN sc.sample_cat='Chemical' AND i.tech_type='PWS' AND wq.parameter='Arsenic'   AND wq.value >= 0.05  THEN 1 ELSE 0 END), 0) AS as_pws,

      /* Manganese exceed by tech */
      COALESCE(SUM(CASE WHEN sc.sample_cat='Chemical' AND i.tech_type='DTW' AND wq.parameter='Manganese' AND wq.value >= 0.4  THEN 1 ELSE 0 END), 0) AS mn_dtw,
      COALESCE(SUM(CASE WHEN sc.sample_cat='Chemical' AND i.tech_type='STW' AND wq.parameter='Manganese' AND wq.value >= 0.4  THEN 1 ELSE 0 END), 0) AS mn_stw,
      COALESCE(SUM(CASE WHEN sc.sample_cat='Chemical' AND i.tech_type='RWH' AND wq.parameter='Manganese' AND wq.value >= 0.4  THEN 1 ELSE 0 END), 0) AS mn_rwhf,
      COALESCE(SUM(CASE WHEN sc.sample_cat='Chemical' AND i.tech_type='PWS' AND wq.parameter='Manganese' AND wq.value >= 0.4  THEN 1 ELSE 0 END), 0) AS mn_pws,

      /* Chloride exceed by tech */
      COALESCE(SUM(CASE WHEN sc.sample_cat='Chemical' AND i.tech_type='DTW' AND wq.parameter='Chloride'  AND wq.value >= 1000 THEN 1 ELSE 0 END), 0) AS cl_dtw,
      COALESCE(SUM(CASE WHEN sc.sample_cat='Chemical' AND i.tech_type='STW' AND wq.parameter='Chloride'  AND wq.value >= 1000 THEN 1 ELSE 0 END), 0) AS cl_stw,
      COALESCE(SUM(CASE WHEN sc.sample_cat='Chemical' AND i.tech_type='RWH' AND wq.parameter='Chloride'  AND wq.value >= 1000 THEN 1 ELSE 0 END), 0) AS cl_rwhf,
      COALESCE(SUM(CASE WHEN sc.sample_cat='Chemical' AND i.tech_type='PWS' AND wq.parameter='Chloride'  AND wq.value >= 1000 THEN 1 ELSE 0 END), 0) AS cl_pws
    ...
    LEFT JOIN sp_san_inspection_v2 si2
     ...
     AND si2.inspection_date BETWEEN :from1 AND :to1

    LEFT JOIN sp_sample_collection sc
      ON sc.infrastructure_id = i.id
     AND sc.sample_date >= :from2
     AND sc.sample_date < DATE_ADD(:to2, INTERVAL 1 DAY)

    LEFT JOIN sp_water_quality wq
      ON wq.sample_id = sc.sample_id
    ...
    ";
        ...
        return [
            ...
            /* Arsenic exceed by tech */
            'as_dtw' => (int)($r ? $r->as_dtw : 0),
            'as_stw' => (int)($r ? $r->as_stw : 0),
            'as_rwhf' => (int)($r ? $r->as_rwhf : 0),
            'as_pws' => (int)($r ? $r->as_pws : 0),

            /* Manganese exceed by tech */
            'mn_dtw' => (int)($r ? $r->mn_dtw : 0),
            'mn_stw' => (int)($r ? $r->mn_stw : 0),
            'mn_rwhf' => (int)($r ? $r->mn_rwhf : 0),
            'mn_pws' => (int)($r ? $r->mn_pws : 0),

            /* Chloride exceed by tech */
            'cl_dtw' => (int)($r ? $r->cl_dtw : 0),
            'cl_stw' => (int)($r ? $r->cl_stw : 0),
            'cl_rwhf' => (int)($r ? $r->cl_rwhf : 0),
            'cl_pws' => (int)($r ? $r->cl_pws : 0),
    ...
    public function weeklyWqAndSiReportExport(Request $request)
    {
        $distid = (int)($request->get('district_id', 6));
        $upazilaId = $request->get('upazila_id');
        $fromDate = $request->get('from_date') ? date('Y-m-d', strtotime($request->get('from_date'))) : "2025-02-01";
        $toDate = $request->get('to_date', date('Y-m-d'));
        ...
        if ($fromDate && $toDate) {
            ...
            $bindings['from_as'] = $fromDate;
            $bindings['to_as'] = $toDate;

            $bindings['from_mn'] = $fromDate;
            $bindings['to_mn'] = $toDate;

            $bindings['from_cl'] = $fromDate;
            $bindings['to_cl'] = $toDate;
        }
        ...
        $sampleData = array_map(function ($r) {
            return [
                ...
                'as_dtw' => (int)$r->arsenic_dtw,
                'as_stw' => (int)$r->arsenic_stw,
                'as_rwhf' => (int)$r->arsenic_rwhf,
                'as_pws' => (int)$r->arsenic_pws,

                'mn_dtw' => (int)$r->manganese_dtw,
                'mn_stw' => (int)$r->manganese_stw,
                'mn_rwhf' => (int)$r->manganese_rwhf,
                'mn_pws' => (int)$r->manganese_pws,

                'cl_dtw' => (int)$r->chloride_dtw,
                'cl_stw' => (int)$r->chloride_stw,
                'cl_rwhf' => (int)$r->chloride_rwhf,
                'cl_pws' => (int)$r->chloride_pws,
            ];
        }, $rows);
